...
默认输出：**完整稿写入文件**（`docs/ux/{模块名}/prd.md`），对话中只呈现摘要和 Handoff。
...
- 检查项目 rules 目录下的 `requirements/`（即当前项目根目录 `./.cursor/rules/requirements/`）是否存在；存在则使用项目要求。
  - 若项目不存在，则检查全局 rules 目录下的 `requirements/`（Cursor：`~/.cursor/rules/requirements/`，Claude：`~/.claude/rules/requirements/`）。存在则使用全局要求；不存在则使用全局 PRD 模板，无需额外操作
  - **存在** → 按以下顺序读取（文件不存在则跳过）：
...
3. 用 Write 工具写入 `docs/ux/{模块名}/prd.md`
3.5 执行「需求匹配度自检」
   ...
   **逻辑一致性检查**：
   ...
   - 权限点与功能需求是否对应
   - 异常边界是否与功能需求中涉及的操作一致
   - **页面清单级联检查**：§4.2 中每个页面条目，在 §4.3（布局要点）、§6.x（功能需求）、§9.x（异常边界）、§11（DoD）、§12（验收用例）中的引用是否一致？若 §4.2 删除或新增了页面，上述章节必须同步，不得残留已删除页面的功能描述或验收用例。
